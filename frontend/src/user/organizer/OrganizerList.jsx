...
import {getOrganizers, deleteOrganizer} from "../../api/organizerService";
...
export default function OrganizerList() {
    ...
    const handleRowClick = React.useCallback(
        ({ row }) => {
            navigate(`/user/organizer/${row.id}`);
        },
        [navigate],
    );

    const handleCreateClick = React.useCallback(() => {
        navigate('/user/organizer/new');
    }, [navigate]);

    const handleRowEdit = React.useCallback(
        (organizer) => () => {
            navigate(`/user/organizer/${organizer.id}/edit`);
        },
        [navigate],
    );
    ...
    const handleRowDelete = React.useCallback(
        (organizer) => async () => {
            ...
            if (confirmed) {
                setIsLoading(true);
                try {
                    await deleteOrganizer(Number(organizer.id));
    ...
    const columns = React.useMemo(
        () => [
            ...
            { field: 'organizerType', headerName: 'Organizer Type', width: 140 },

...
export default function AdminList() {
    ...
    const handleRowClick = React.useCallback(
        ({ row }) => {
            navigate(`/user/admin/${row.id}`);
        },
        [navigate],
    );

    const handleCreateClick = React.useCallback(() => {
        navigate('/user/admin/new');
    }, [navigate]);

    const handleRowEdit = React.useCallback(
        (admin) => () => {
            navigate(`/user/admin/${admin.id}/edit`);
        },
        [navigate],
    );
    ...
    const handleRowDelete = React.useCallback(
        (admin) => async () => {
            ...
            if (confirmed) {
                setIsLoading(true);
                try {
                    await deleteAdmin(Number(admin.id));
    ...
    const columns = React.useMemo(
        () => [
            ...
            { field: 'adminRole', headerName: 'Admin Role', width: 140 },

...
export default function StudentList() {
  ...
  const handleRowClick = React.useCallback(
    ({ row }) => {
      navigate(`/user/student/${row.id}`);
    },
    [navigate],
  );

  const handleCreateClick = React.useCallback(() => {
    navigate('/user/student/new');
  }, [navigate]);

  const handleRowEdit = React.useCallback(
    (student) => () => {
      navigate(`/user/student/${student.id}/edit`);
    },
    [navigate],
  );
  ...
  const handleRowDelete = React.useCallback(
    (student) => async () => {
      const confirmed = await dialogs.confirm(
        `Do you wish to delete ${student.name}?`,
        {
      ...
      if (confirmed) {
        ...
        try {
          ...
          notifications.show(
            `Failed to delete student. Reason:' ${deleteError.message}`,
            {
              severity: 'error',
              autoHideDuration: 3000,
            },

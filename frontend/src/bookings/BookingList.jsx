...
export default function BookingList() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setRows((Array.isArray(data) ? data : []).map(normalizeId));
        } catch (e) {
            console.error(e);
            setErr("Failed to load bookings. Check your backend and API baseURL.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAdd = () => navigate("/bookings/new");
    const handleEdit = () => {
        if (selection.length !== 1) return;
        navigate(`/bookings/${selection[0]}/edit`);
    };
    const handleDelete = async () => {
        if (selection.length < 1) return;
        if (!window.confirm(`Delete ${selection.length} booking(s)?`)) return;
        setBusy(true);
        try {
            ...
            console.error(e);
            setErr("Failed to delete booking(s).");
        } finally {
            setBusy(false);
        }
    };
    ...
    const columns = useMemo(
        () => [
            ...
            { field: "bookingID", headerName: "Booking ID", width: 140, valueGetter: (p) => p.row.bookingID ?? p.row.id },
